...
with PowerProfiler(handle):
    model = AutoModelForCausalLM.from_pretrained(MODEL_DIR, torch_dtype="auto", device_map="cuda",use_kernels=True,allow_all_kernels=True)
    model.eval()
    ...
    with PowerProfiler(handle):
        for i in range(601):
            ...
            with torch.no_grad():
                ...
                # print(f"Decoded: {tokenizer.decode(predicted)!r}")
...
# run_prefill("Hello")
run_prefill("Hello there world! It is so nice to meet you all!")